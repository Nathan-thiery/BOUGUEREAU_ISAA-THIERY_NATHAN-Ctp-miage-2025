...
#Nathan THIERY# et #Isaac BOUGUEREAU#
...
Le code dans todolist-debut-ex1 n'est pas structuré en couches ; l'ensemble des classes se trouve dans le même dossier. Cela peut conduire à un couplage fort entre la logique métier, la persistance et les aspects de présentation, ce qui rendra difficile la maintenance, l'évolution et la réutilisation du code si le projet grandit.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier se trouve dans la classe TodoItem, qui implémente notamment la règle RG 1 via la méthode finalContent.

*Cette règle est-elle facilement testable par un test unitaire ?*
Cette règle est facilement testable en test unitaire car elle est encapsulée dans la classe métier (TodoItem) sans dépendances externes, ce qui permet de lui fournir des entrées et vérifier sa sortie de manière isolée.
...
Cette commande Docker fait ce qui suit :

docker run : lance un nouveau conteneur.
-it : active le mode interactif et ouvre un terminal pour pouvoir voir l'activité du conteneur en direct.
-e POSTGRES_PASSWORD=password : définit la variable d'environnement POSTGRES_PASSWORD dans le conteneur avec la valeur password, ce qui configure le mot de passe de l'utilisateur postgres par défaut.
-p 5432:5432 : publie le port 5432 du conteneur sur le port 5432 de la machine hôte, permettant aux applications externes de se connecter (ici, la base PostgreSQL).
postgres : indique l'image Docker à utiliser, ici l'image officielle de PostgreSQL.
...
todoItemRepository est null car il n'est pas injecté automatiquement par le framework Spring. La meilleure façon de l'injecter est d'utiliser l'injection de dépendances par constructeur, ce qui garantit que le bean est fourni par le contexte Spring au moment de l'instanciation du contrôleur. Par exemple :
Ainsi, Spring injectera automatiquement une instance de TodoItemRepository lorsqu'il créera le bean TodoListController.
